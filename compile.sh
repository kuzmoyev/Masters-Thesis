...
lilypond-book --output=${OUT_DIR} --pdf ${TEX_FILE}
cp -R *.cls *.bib *.bst cvut-logo-bw-en.pdf img ${OUT_DIR}/
cd ${OUT_DIR}
mkdir -p ${OUT_DIR}
pdflatex -halt-on-error -file-line-error -interaction=nonstopmode -output-directory=${OUT_DIR} "${TEX_FILE}" > ${LOG_FILE}
